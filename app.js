...
function renderAccountArea(){
  ...
  img.src = currentUser.avatar || 'logo.png';
...
function renderSidebarProfile(){
  ...
  sideAvatarImg.src = currentUser.avatar || 'logo.png';
...
function openProfileModal(){
  if(currentUser){
    ...
    pfAvatarPreview.src = currentUser.avatar || 'logo.png';
  } else {
    pfName.value = '';
    pfHandle.value = '';
    pfAvatarPreview.src = 'logo.png';
  }
...
function makePostElement(post){
  ...
  el.innerHTML = `
    <div class="meta">
      <div class="avatar"><img src="${post.author.avatar || 'logo.png'}" alt="avatar" style="width:100%;height:100%;border-radius:10px;object-fit:cover"></div>
...
function renderBadges(author){
  ...
  if(author.admin){
    ...
    parts.push(`<span class="badge admin-logo" title="Official Administrator"><img src="logo.png" alt="logo"></span>`);
  }
  if(author.verified){
...
if(posts.length===0){
  posts.push({id: Date.now()+1, text:'Exploring 3D transforms for a more tactile feed!', author:{name:'Admin',handle:'@admin',avatar:'logo.png',admin:true,verified:true}});
  posts.push({id: Date.now()+2, text:'CSS-only depth can feel surprisingly immersive.', author:{name:'Designer',handle:'@designer',avatar:'logo.png',admin:false,verified:false}});
  posts.push({id: Date.now()+3, text:'Micro-interactions make browsing delightful.', author:{name:'Creator',handle:'@creator',avatar:'logo.png',admin:false,verified:true}});
  savePosts();
}
...
function handleMenuAction(action){
  ...
  if(action === 'explore'){
    ...
    trending.forEach(t=>{
      ...
      el.innerHTML = `<div class="meta"><div class="avatar"><img src="logo.png" style="width:100%;height:100%;object-fit:cover;border-radius:10px"></div><div class="author"><strong><span class="display-name">OMGSocial</span>${renderBadges({admin:true,verified:false})}</strong><div class="handle">@omgsocial</div></div></div><div class="content">${escapeHtml(t)}</div>`;
